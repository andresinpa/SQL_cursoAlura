...
SELECT CIUDAD, BARRIO, MAX(LIMITE_DE_CREDITO) AS MAXIMO_CREDITO FROM tabla_de_clientes WHERE CIUDAD = 'CIUDAD DE MEXICO' GROUP BY BARRIO;

SELECT ESTADO, BARRIO, MAX(LIMITE_DE_CREDITO) AS MAXIMO_CREDITO FROM tabla_de_clientes GROUP BY ESTADO, BARRIO;

SET GLOBAL sql_mode=(SELECT REPLACE(@@sql_mode,'ONLY_FULL_GROUP_BY',''));

SELECT ESTADO, BARRIO, MAX(LIMITE_DE_CREDITO) AS LIMITE, EDAD FROM tabla_de_clientes WHERE EDAD >=21 GROUP BY ESTADO, BARRIO ORDER BY EDAD;
